...
/*----------------------------------------------------------------------------*/
/**
 * Sample code for MidField API: DesktopServer
 *
        ...
        CommPacketHandler
{
    ...
    private static final String STR_ACCEPT_CONTROL = "※遠隔(%s)からのデスクトップ操作を開始します．";
    private static final String STR_REJECT_CONTROL = "※遠隔(%s)からのデスクトップ操作を拒否します．";
    private static final String STR_STOP_CONTROL   = "※遠隔からのデスクトップ操作を終了します．";
    
    private static final String STR_CANT_CREATE_ROBOT = "デスクトップ操作用のインスタンスを生成できません．";
    private static final String STR_UNREACHABLE       = "クライアントへ送信したパケットが到達できませんでした";
...
// -----------------------------------------------------------------------------
    ...
    public synchronized boolean handleIncomingPacket(CommPacket pkt)
    {
        ...
        switch (type) {
        case START_CONTROL:
            msgHn_StartControl(msg, srcId);
            break;
        case CONTROL_MESSAGE:
            msgHn_ControlMessage(msg);
            break;
        case STOP_CONTROL:
            msgHn_StopControl(msg);
            break;
        default:
...
// -----------------------------------------------------------------------------
        ...
        throws SystemException
    {
        ...
        try {
            ...
            SystemException sysEx = new SystemException(STR_CANT_CREATE_ROBOT, ex);
            throw sysEx;
        }
    }
...
// -----------------------------------------------------------------------------
    ...
    private void msgHn_ControlMessage(DesktopMessage msg)
    {
        ...
        switch (action) {
        case KEY_PRESS:
            evHn_KeyPress((KeyEvent)obj);
            break;
        case KEY_RELEASE:
            evHn_KeyRelease((KeyEvent)obj);
            break;
        case MOUSE_PRESS:
            evHn_MousePress((MouseEvent)obj);
            break;
        case MOUSE_RELEASE:
            evHn_MouseRelease((MouseEvent)obj);
            break;
        case MOUSE_MOVE:
            evHn_MouseMove((MouseEvent)obj);
            break;
        case MOUSE_WHEEL:
            evHn_MouseWheel((MouseWheelEvent)obj);
            break;
        case UNKNOWN:
            /* error */ break;
        }
...
// -----------------------------------------------------------------------------
    ...
    private int getMouseButtonNumber(MouseEvent ev)
    {
        ...
        case MouseEvent.BUTTON1:
            btnNum = InputEvent.BUTTON1_DOWN_MASK;
            break;
        case MouseEvent.BUTTON2:
            btnNum = InputEvent.BUTTON2_DOWN_MASK;
            break;
        case MouseEvent.BUTTON3:
            btnNum = InputEvent.BUTTON3_DOWN_MASK;
            break;
        default:
            /* error */ break;
        }
